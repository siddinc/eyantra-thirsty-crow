...
from constants import INVERSE_MATRIX, marker_length
...
def init_gl():
        ...
        crow = OBJ('low_water.obj', swapyz=True)
"""
...
def drawGLScene():
        ...
        if ret == True:
                ...
                for i in ar_list:
                        ...
                        frame = aruco.drawAxis(frame, camera_matrix, dist_coeff, i[2], i[3], marker_length) # DEBUG ONLY
...
def detect_markers(img):
        ...
        if type(ids) == np.ndarray:
                for i in range(len (ids)):
                    rvec, tvec, _ = aruco.estimatePoseSingleMarkers(corners[i], marker_length, camera_matrix, dist_coeff)
